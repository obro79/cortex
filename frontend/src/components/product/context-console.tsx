...
import { requestTaskContext, type TaskContextResponse } from "@/lib/cortex-api";
...
export function ContextConsole() {
  ...
  return <main className="mx-auto max-w-6xl p-6">
    ...
    {result && <section className="cortex-panel mt-4 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-sm font-semibold">Contract result</h2><span className="rounded border border-border px-2 py-1 font-mono text-xs">{result.status}</span></div>{result.ok ? <><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-xs text-muted-foreground">Data mode</dt><dd className="mt-1">{result.live_data ? "live" : "fixture or imported snapshot"}</dd></div><div><dt className="text-xs text-muted-foreground">Evidence</dt><dd className="mt-1">{result.task_context.source_coverage.evidence_item_count}</dd></div><div><dt className="text-xs text-muted-foreground">Freshness</dt><dd className="mt-1">{result.task_context.freshness.status}</dd></div><div><dt className="text-xs text-muted-foreground">Retrieval</dt><dd className="mt-1">{result.task_context.retrieval.status}</dd></div></dl><div className="mt-4 border-t border-border pt-4 text-sm"><p className="text-xs text-muted-foreground">Trace ID</p><code>{result.trace_id}</code>{result.evidence_pack_id && <Link className="ml-4 text-cortex-blue" href={`/ui/evidence/${result.evidence_pack_id}`}>Inspect evidence pack →</Link>}</div></> : <><p className="mt-3 text-sm text-muted-foreground">{result.error.message}</p><p className="mt-2 font-mono text-xs text-muted-foreground">{result.error.code} · trace {result.trace_id}</p></>}</section>}
  </main>;
}
