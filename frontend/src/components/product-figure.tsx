...
export function ProductFigure({ figure, title, children }: ProductFigureProps) {
  return (
    <figure className="border-y border-cortex-border bg-cortex-panel/70 px-4 py-5 sm:px-6 lg:px-8">
      <figcaption className="mb-4 flex items-center justify-between gap-4 text-xs text-cortex-muted">
        <span>{figure}</span>
        <span>{title}</span>
      </figcaption>
      {children}
