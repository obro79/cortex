...
class SlackWebhookService:
    ...
    async def handle(
        ...
        *,
        ...
        delivery, created = self.deliveries.create_or_duplicate(
        ...
        source = self.source_connections.get_selected_channel(workspace_id, channel_id)
        if source is None:
        ...
        team_id = str(source.provider_metadata_json.get("team_id", "team"))
        result = await self.ingestion.ingest(
            RawEventInput(
                ...
                idempotency_key=f"slack:{workspace_id}:{channel_id}:{event_ts}:{event_type}",
                payload=payload,
